        ...
        case TA_TXPOLL_WAIT_SEND :
            {
				...
				if(is_button_low(0)==S1_SWITCH_ON)
					memcpy(&inst->msg.destAddr[0], &inst->payload.anchorAddressList[inst->anchor_id], ADDR_BYTE_SIZE_L);
				else //set destination address
					if(destaddress(inst))
					{
						break;
					}
        ...
        case TA_TXREPORT_WAIT_SEND :
            {
                ...
                if(inst->newReportSent == 0) //keep the same message if re-sending the same report
                {
					...
					memcpy(&(inst->msg.messageData[TOFR]), &inst->tof, 5);//yavari you should send the position using this message data.
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        switch(fcode)
                        {
                            ...
                            case RTLS_DEMO_MSG_ANCH_TOFR:
                            {
								...
								memcpy(&inst->tof, &(messageData[TOFR]), 5); //yavari you will receive the position report here
...
// -------------------------------------------------------------------------------------------------------------------
//
// Set next anchor ID //yavari
//
// -------------------------------------------------------------------------------------------------------------------
void instancesetNextAnchId(int id)
{
	int instance = 0;
	instance_data[instance].anchor_id = id;
}
// -------------------------------------------------------------------------------------------------------------------
//
